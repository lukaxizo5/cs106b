...
//using password from HuffmanEncoding.cpp
extern string password;
//using the variable to store if user has provided the correct password.
bool safe = false;
...
void testCompleteStack() {
	...
	if (count <= 0) {
		cout << "Did not test, because you did not provide correct password!" << endl;
		return;
	}
...
int main() {
	testEnvironment();
	cout << "Enter your password for compressing and decompressing" << endl; 
	cout <<	"If you will not, the password will be : " << endl;
	cout << "KHIZAMBARELI" << endl;
	askForPassword();
	while (true) {
		displayMenu();
